<template>
    <div>
        <jet-form-section @submitted="updateExpense">
            <template #title>
                Edit Expense
            </template>
...
export default {
    ...
    data() {
        return {
            form: this.$inertia.form(
                {
                    _method: "PATCH",
                    id: this.$page.expense.id,
                    description: this.$page.expense.description,
                    amount: this.$page.expense.amount,
                    method: this.$page.expense.method,
                    method_remarks:this.$page.expense.method_remarks,
                    client_id: this.$page.expense.client_id,
                },
                {
                    bag: "updateExpense",
    ...
    methods: {
        ...
        updateExpense() {
            this.form.post("/expenses/" + this.$page.expense.id, {
                preserveScroll: true
            });
        },
        ...
        deleteApiToken() {
            this.$inertia
                .delete("/expenses/" + this.$page.expense.id, {
                    preserveScroll: true
                })
                .then(() => (window.location = this.$route("expenses.index")));
        },
    ...
    created() {
        this.form.method = { id:1, value: this.$page.expense.method}
    }
};
</script>
